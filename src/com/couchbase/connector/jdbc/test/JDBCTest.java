        ...
        while (rs.next()) {
            for (int i = 1; i < columnsNumber; i++) {
                if (rsmd.getColumnLabel(i).equals("TABLE_NAME")) {
                    ...
                    if (tableName.contains("-") || tableName.contains("default")) {
                        continue;
                    }
                    ResultSet schema = metadata.getColumns(null, null, tableName, null);
                    ResultSetMetaData schemaMetadata = schema.getMetaData();
                    while (schema.next()) {
                        for (int j = 1; j <= schemaMetadata.getColumnCount(); j++) {
                            System.out.print(schemaMetadata.getColumnLabel(j) + ":" + schema.getString(j) + "|");
                        }
                        System.out.println();
                    }
                    System.out.println();
